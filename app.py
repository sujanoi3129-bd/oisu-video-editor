...
st.write("মিনিট এবং সেকেন্ড হিসাব করে নিখুঁতভাবে ভিডিও কাটার স্পেশাল আপডেট ভাই!")
...
# ==========================================
# 🟢 EDP ১: ভিডিও আপলোড ও কপিরাইট রিমুভ
# ==========================================
...
# ==========================================
# 🟢 EDP ২: ভিডিও দেখে মিনিট-সেকেন্ডে কাটা
# ==========================================
elif st.session_state.step == 2:
    st.header("Step ২: ভিডিও কাটিং টাইমলাইন (মিনিট ও সেকেন্ড)")
    ...
    if st.session_state.video_data is not None:
        ...
        # সেকেন্ডকে মিনিট এবং সেকেন্ডে রূপান্তর (অটোমেটিক শো করার জন্য)
        max_mins = int(total_seconds // 60)
        max_secs = int(total_seconds % 60)
        
        st.markdown(f"🎒 **আপনার আপলোড করা ভিডিওর মোট সময়:** `{max_mins}` মিনিট `{max_secs}` সেকেন্ড।")
        st.info("💡 নিচে আপনি কত মিনিট কত সেকেন্ড থেকে কত মিনিট কত সেকেন্ড পর্যন্ত রাখতে চান তা বসিয়ে দিন:")

        # ইনপুট বক্স জোড়া (মিনিট এবং সেকেন্ড আলাদা)
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("⏱️ যেখান থেকে কাটা শুরু হবে:")
        ...
        with col2:
            st.subheader("⏱️ যেখানে গিয়ে কাটা শেষ হবে:")
        ...
        if st.button("✂️ ২. ভিডিও কাটুন"):
            # ইনপুট করা মিনিট-সেকেন্ডকে আবার এফএফএমপেগের জন্য সেকেন্ডে রূপান্তর
            final_start_seconds = (start_m * 60) + start_s
            final_end_seconds = (end_m * 60) + end_s
            
            if final_end_seconds <= final_start_seconds:
                st.error("❌ শেষের সময় অবশ্যই শুরুর সময়ের চেয়ে বেশি হতে হবে সুজন ভাই!")
                ...
                with st.spinner("আপনার দেওয়া মিনিট ও সেকেন্ড অনুযায়ী ভিডিও কাটা হচ্ছে..."):
                    ...
                    # নিখুঁত কাটিংয়ের জন্য টাইম ফরম্যাট তৈরি (HH:MM:SS)
                    ...
                    if os.path.exists(v_step2) and os.path.getsize(v_step2) > 0:
                        with open(v_step2, "rb") as f:
                            st.session_state.video_data = f.read()
                        st.success(f"✅ ধাপ ২ সফল! ভিডিওটি {start_m}মি:{start_s}সে থেকে {end_m}মি:{end_s}সে পর্যন্ত কাটা হয়েছে।")
    ...
    if st.button("🎬 ৪. ফাইনাল ভিডিও তৈরি ও সেভ করুন"):
        if st.session_state.video_data is not None:
            ...
            if os.path.exists(v_final) and os.path.getsize(v_final) > 0:
                st.success("🎉 আলহামদুলিল্লাহ সুজন ভাই! মিনিট-সেকেন্ড ট্র্যাকিং, কাটিং, নাম সব একসাথে ১০০% সাকসেস হয়েছে।")
                ...
                with open(v_final, "rb") as file:
                    st.download_button(
                        ...
                        data=file,
                        file_name="sujon_minute_fixed_video.mp4",
                        mime="video/mp4"
                    )
                if os.path.exists(v_step3): os.remove(v_step3)
            else:
